...
const REGISTRY_BASE_URL = process.env.REGISTRY_BASE_URL || "http://localhost:3000" ;
...
const parseContent = (content: string) =>
  content
    .replace(/\\/g, "\\\\")
    .replace(/"/g, '\\"') // Add this line to escape double quotes
    .replace(/`/g, "\\`")
    .replace(/\${/g, "\\${")
    .replace(/\n/g, "\\n")
    .trim();
...
export const Index: Record<string, any> ={`;
  ...
  const registryJSON = JSON.stringify(
    {
      $schema: `${REGISTRY_BASE_URL}/schema/registry.json`,
      name: registry.name,
      ...
      items: registry.items
        ...
        // Transform registryDependencies to use custom registry
